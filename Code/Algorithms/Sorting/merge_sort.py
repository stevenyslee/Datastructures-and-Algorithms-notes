...
# Since we cover 0 ^ 0 case in the beginning of the loop,while bool(a) or bool(b) makes 0^0 is impossible to happen, Xor has 3 cases, 1 ^ 0, 0 ^ 1, 1^1 to evaluate. Since the 2 first cases are the edge we go inside the loop and alias the array that has elements in it. If it's 1^1 we simply pick the array with the smallest element.
